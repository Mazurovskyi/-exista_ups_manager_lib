...
/*
    ...
    'reading: loop{
        match port.read(&mut buf){
            ...
            Ok(_) => {
                println!("0 (zero) bytes have received from buffer. There are no avaliable data in buffer or 
            ...
            Err(err)=>{
                match err.kind(){
                    ErrorKind::Interrupted => {
                        println!("ErrorKind::Interrupted. try to read buffer again.");
                        continue 'reading
                    }
                    _=> continue 'reading
                }
...
pub fn new(addr:u8, code:u8, offset:u16, count:u16, rev: u8)->[u8; 8]{
...
mod tests {
    ...
    fn check_reverse(){
        let rev = 1;
        let get_current =    new(0x11, 0x03, 0x12, 0x01, rev);
        let handshake =      new(0x01, 0x06, 0x50, 0x00, rev);
